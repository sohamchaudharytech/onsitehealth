...
import { loadDomainSnapshot, loadLedgerBlocks, persistDomainSnapshot, persistLedgerBlock, persistRefreshEvent, loadRefreshEvents, } from './persistence.js';
...
const requestOriginALS = new AsyncLocalStorage();
const macCache = new Map();
const MAC_CACHE_MS = 60_000;
async function resolveMacFor(ip) {
    ...
    if (cached && Date.now() - cached.at < MAC_CACHE_MS)
        return cached.mac;
    let mac = null;
    try {
        const { execFile } = await import('node:child_process');
        const out = await new Promise((resolve, reject) => {
            execFile('arp', ['-n', clean], (err, stdout) => (err ? reject(err) : resolve(stdout)));
        });
...
let localMacCache;
function localMacSync() {
    if (localMacCache !== undefined)
 ...
 */
function stampOrigin(target, ip, mac) {
    if (ip)
        target.ip = ip.replace(/^::ffff:/, '');
    if (mac)
        target.mac = mac;
    return target;
}
...
let domainPersistenceTimer = null;
let domainPersistenceInFlight = false;
function flushDomainSnapshot() {
    if (domainPersistenceInFlight || !domainPersistenceEnabled)
        return;
...
function scheduleDomainSnapshot() {
    if (!domainPersistenceEnabled)
        return;
    if (domainPersistenceTimer)
        clearTimeout(domainPersistenceTimer);
...
const simSites = new Map();
const simCaches = new Map();
const simEvaluators = new Map();
const simResults = new Map();
let nextSimPortOffset = 0;
async function hostSimulatedHospital(siteId) {
    ...
    const sim = await startSimSite(siteId, port, {
        push: (ruleVersion) => simCaches.get(siteId).ingest(ruleVersion),
...
async function teardownSimulatedHospital(siteId) {
    const sim = simSites.get(siteId);
    if (sim) {
        await new Promise((resolve) => sim.handle.close(() => resolve()));
...
async function replayHistoryToSite(site) {
    const rules = store.allRules();
    await Promise.all(rules.map((rec) => pushToSite({ store, ledger, broadcast }, site, rec).then((outcome) => {
...
app.patch('/api/sites/:siteId/network', requirePermission('sites:manage'), (req, res) => {
    ...
    if (patch.baseLatencyMs !== undefined)
        clean.baseLatencyMs = Math.max(0, Number(patch.baseLatencyMs));
    if (patch.jitterMs !== undefined)
        clean.jitterMs = Math.max(0, Number(patch.jitterMs));
    if (patch.dropRate !== undefined)
        clean.dropRate = Math.min(1, Math.max(0, Number(patch.dropRate)));
...
app.get('/api/hospitals', (_req, res) => {
    ...
    for (const d of users.listDoctors()) {
        if (d.hospitalId)
            doctorCountByHospital.set(d.hospitalId, (doctorCountByHospital.get(d.hospitalId) ?? 0) + 1);
    }
...
function patientView(p, hospitals) {
    return {
        patientId: p.patientId,
        email: p.email,
        data: p.data,
...
app.get('/api/patients', requirePermission('patients:manage'), (req, res) => {
    ...
    const hospitals = new Map(store.listHospitals().map((h) => [h.siteId, h.name]));
    const list = patients.list().map((p) => ({
        patientId: p.patientId,
        email: p.email,
        data: p.data,
        createdAt: p.createdAt,
...
app.patch('/api/patients/:patientId', requirePermission('patients:manage'), (req, res) => {
    ...
    if (typeof email === 'string' && EMAIL_RE.test(email.trim()) && email.trim().toLowerCase() !== rec.email) {
        ...
        try {
            ...
            if (portal)
                users.updateUsername(portal.userId, mail);
...
app.post('/internal/evaluations', internalKeyGuard(INTERNAL_KEY), (req, res) => {
    ...
    for (const r of results) {
        ledger.append('ORDER_EVALUATED', r);
    }
...
app.post('/api/orders', requirePermission('orders:submit'), async (req, res) => {
    ...
    const sites = store.listSites().filter((s) => !siteIds || siteIds.includes(s.siteId));
    if (sites.length === 0) {
    ...
    const evaluations = await Promise.all(sites.map(async (site) => {
        try {
            ...
            if (!r.ok)
                throw new Error(`site ${site.siteId} HTTP ${r.status}`);
...
const shutdown = () => {
    flushDomainSnapshot();
    for (const sim of simSites.values())
        sim.handle.close();
