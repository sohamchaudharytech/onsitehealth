...
import { defaultRateLimitRules, errorHandler, internalKeyGuard, jwtAuth, requestLogger, requirePermission, sanitizeBody, signJwt, SlidingWindowLimiter, verifyJwt, } from '@hc/shared';
...
function seedShipments() {
    const now = Date.now();
    const mk = (orderCode, drugName, quantity, coldChain, origin, destination, status, minutesAgo) => {
        ...
        if (status !== 'NOT_SENT') {
            ...
            if (from)
                from.shipment.lastEvent = null; // seed quietly
            if (status === 'DELIVERED') {
                const d = shipments.transition(s.shipmentId, 'DELIVERED', null);
                if (d)
                    d.shipment.lastEvent = null;
            }
        }
        return s;
    };
    ...
    for (const [code, fraction] of [['RX-2026-0001', 0.15], ['RX-2026-0002', 0.45], ['RX-2026-0003', 0.75]]) {
...
app.post('/api/shipments/:shipmentId/status', requirePermission('orders:submit'), (req, res) => {
    ...
    const t = shipments.transition(s.shipmentId, to, {
