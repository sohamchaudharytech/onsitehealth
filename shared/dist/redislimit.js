...
import { SlidingWindowLimiter, defaultRateLimitRules } from './ratelimit.js';
/**
...
export class RedisSlidingWindowLimiter {
    redis;
    fallback = new SlidingWindowLimiter(defaultRateLimitRules());
    ...
    offenses = new Map();
    rules;
    constructor(rules = defaultRateLimitRules(), opts = {}) {
        this.rules = rules;
    ...
    async redisCheck(key, windowMs, limit, hit) {
        try {
            const nowUs = Date.now() * 1000;
            const windowUs = Math.round(windowMs * 1000);
            const res = await this.redis.eval(hit ? CHECK_AND_HIT_LUA : PEEK_LUA, [`rl:${key}`], [nowUs, windowUs, limit]);
...
//# sourceMappingURL=redislimit.js.map